...
export default function UsaGlobe() {
  const globeConfig = {
    ...
    rings: 1,
    maxRings: 1,
    initialPosition: { lat: 38.0, lng: -97.0 }, // Center roughly over the USA
    autoRotate: false,
    autoRotateSpeed: 0.5,
  };
...
// World Component (World.tsx)
import { useEffect, useRef, useState, useLayoutEffect } from "react";
import { Color, Scene, Fog } from "three";
import ThreeGlobe from "three-globe";
import { useThree, Object3DNode, Canvas, extend } from "@react-three/fiber";
import { OrbitControls } from "@react-three/drei";
const countries = require("/components/data/globe.json"); // Adjust the path as needed
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  // Build data and material when globeRef is available
  useEffect(() => {
    if (globeRef.current) {
      _buildData();
      _buildMaterial();
  ...
  // Apply globe data and set initial rotation
  ...
  return (
    <>
      {<threeGlobe ref={globeRef} />} {/* Conditional Rendering */}
    </>
  );
}
