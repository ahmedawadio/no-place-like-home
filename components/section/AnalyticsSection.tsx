...
export default function AnalyticsSection({locationData, setLocationData}: Props) {
  ...
  useEffect(() => {
    ...
    if (!locationData.zipcode.name?.length) {
  ...
  if (!locationData.initial_zipcode.length) {
    return (
      <div className="bg-black h-screen w-screen flex items-center justify-center">
        <div className="flex items-center space-x-4" role="status">
          {isTimeout ? (
            // Show "Try again later" after 10 seconds
            <p className="text-white text-2xl font-medium opacity-80">Try again later...</p>
          ) : (
            <>
              {/* Loading spinner with transparency */}
              <svg
                aria-hidden="true"
                className="w-7 h-7 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600 opacity-60"
                viewBox="0 0 100 101"
                fill="none"
              ...
              {/* Loading text with transparency */}
              <p className="text-white text-2xl font-medium opacity-60">Loading...</p>
            </>
          )}
        </div>
      </div>
